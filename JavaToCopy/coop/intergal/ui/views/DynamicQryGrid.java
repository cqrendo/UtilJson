...
@CssImport(value = STYLES_CSS, themeFor = "dynamic-qry-grid")
public class DynamicQryGrid extends PolymerTemplate<TemplateModel> implements BeforeEnterObserver, HasDynamicTitle{//, PageConfigurator{//, VaadinServiceInitListener  {
...
//      refreshLog();
	...
	public DynamicQryGrid(TemplateParser parser, VaadinService service) {
		super(parser, service);
	}

	public DynamicQryGrid(TemplateParser parser) {
		super(parser);
	}
...
//				displayFormClassName = PACKAGE_VIEWS+queryParameters.getParameters().get("displayFormClassName").get(0);
		...
		try {
			...
			Method setFilter = dynamicQuery.getMethod("setFilter", new Class[] {String.class} );

			setGrid.invoke(queryForm,grid);
			setFilter.invoke(queryForm,filter);
			divQuery.removeAll();
...
//		grid.setButtonsForm(buttons);
		...
		grid.setCache(cache);
		grid.setupGrid(true, true, true, false, false);
